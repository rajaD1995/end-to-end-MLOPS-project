'''
Data is loaded from github or s3. Then preprocessing & then split into
train & test and then save the train and test in data/raw folder.
The split values is take from config.yaml file -src/connections/config.json
'''
...
def save_data(train_data: pd.DataFrame, test_data: pd.DataFrame, data_path: str) -> None:
    ...
    end-to-end-MLOPS-project/data/raw
    """
    try:
...
def main():
    try:
        ...
        # If yaml file is not written
        # test_size = 0.2

        # If no connection with s3. Data taken from github url
        df = load_data(data_url='https://raw.githubusercontent.com/vikashishere/Datasets/refs/heads/main/data.csv')

        # If connection s3 has estabilished
