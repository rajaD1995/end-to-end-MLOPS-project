...
def apply_bow(train_data: pd.DataFrame, test_data: pd.DataFrame, max_features: int) -> tuple:
    ...
    try:
        ...
        # Seperate the independent vatiables to apply vectorizer
        X_train = train_data['review'].values
        y_train = train_data['sentiment'].values
        X_test = test_data['review'].values
        y_test = test_data['sentiment'].values

        # applyting vectorizer on independent vatiables
        X_train_bow = vectorizer.fit_transform(X_train)
        X_test_bow = vectorizer.transform(X_test)

        # convert the new 50 or 100 features in to data frame and add the dependent variable
        ...
        # Vectorizer model is saved
        # It will be used, when user give input(review), we have to convert that input
        # into vectores in the same way.
...
def main():
    try:
        ...
        # train & test df is saved.
        # both has now features = max_features + dependend variable
